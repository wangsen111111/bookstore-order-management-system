...
public class BillDaoImpl implements BillDao {
    ...
    public int modify(Connection connection, Bill bill) throws Exception {
        ...
        PreparedStatement pstm = null;
        if (null != connection) {
            String sql = "update `bookstore order management system_bill` set productName=?," +
    ...
    public int getBillCountByProviderId(Connection connection, String providerId) throws Exception {
        ...
        ResultSet rs = null;
        if (null != connection) {
            String sql = "select count(1) as billCount from `bookstore order management system_bill` where" +
                    "	providerId = ?";
